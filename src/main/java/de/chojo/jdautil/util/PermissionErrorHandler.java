...
public final class PermissionErrorHandler {
    ...
    public static void handle(InsufficientPermissionException permissionException, ShardManager shardManager,
                              ILocalizer localizer, List<Long> ignoredGuilds) {
    ...
    public static void sendPermissionError(GuildMessageChannel channel, Permission permission, ILocalizer localizer,
        ...
        if (ignoredGuilds.contains(guild.getIdLong())) return;
     ...
     * @param ignoredGuilds guild where no error handling should be done.
     * @param permissions   permissions to check
     * @return true if a permission was missing and a message was send
     */
    public static boolean assertAndHandle(GuildMessageChannel channel, ILocalizer localizer, List<Long> ignoredGuilds, Permission... permissions) {
        try {
            assertPermissions(channel, permissions);
        } catch (InsufficientPermissionException e) {
            handle(e, channel, localizer, ignoredGuilds);
            return true;
        }
        return false;
    }
